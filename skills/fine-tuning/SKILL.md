...
3. EXCEPTION TO RULE 1: never call submit_finetune_run in the same turn as the initial fine-tuning request, for any reason, including to "check" whether the dataset/config is valid — that speculative call IS the forbidden action, whether or not it succeeds. Use get_dataset_job_status, or the datasets skill's `resources_list` check for what's already staged, instead to check preconditions. Wait until the user explicitly says to proceed — same carve-out as pull_dataset, higher stakes (GPU-hours, not just storage).
