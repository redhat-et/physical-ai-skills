...
4c. Before passing a reference-derived number into expected_action_dim/expected_exterior_cameras/expected_wrist_cameras, check HOW the reference states it: a number given as a hard requirement is fine to pass through, but a number mentioned only to describe internal recipe behavior (e.g. "auto-pads to N", "not required", "no fixed requirement") means that parameter has no correct value to pass at all -- omit it, don't pass the number anyway. validate_dataset always reports a raw-value mismatch as flatly "INCOMPATIBLE" with no way to soften it, so passing a number the reference itself says isn't binding produces a confident-looking but false blocker (confirmed: this happened twice in a row for pi0.5's action dimensionality, which auto-pads to 32 regardless of the dataset's raw shape). Also don't assume every checklist parameter bundled into one Dimension row inherits that row's single Priority label uniformly -- a row's Priority can be qualified in parentheses (e.g. "Critical (encoding only)", "Critical (version only)") to flag that only one sub-fact in that row is the actual blocker; read the qualifier and the row's full prose, not just the headline word, before treating a validate_dataset mismatch as disqualifying.
...
|---|---|---|---|
| 1 | Embodiment & Kinematics | Arm/platform, DoF, kinematic chain, gripper type, gripper encoding + polarity, joint/workspace limits | `robot_type` from validate_dataset (often unpopulated) + the Features list. DoF/kinematics/limits: no tool exposes these — use the target model's model-specs reference or ask the user. |
...
| 8 | Scale & Composition | Total episodes/frames/hours, minimum viable episode count, storage size, success/fail composition | get_dataset_info(view='summary') reports GB; validate_dataset reports `total_episodes` — check both against what the recipe needs (per its model-specs reference). |
